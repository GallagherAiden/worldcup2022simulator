...
function watchMatch() {
	...
	document.getElementById("awayout").innerHTML = "";
	document.getElementById("output").innerHTML = "";
	document.getElementById("pitch").innerHTML = '<canvas id="map" style="border:1px solid lime; background: url(\'img/pitch.jpg\'); background-size: 100% 100%;">';
...
function movePlayers() {
	...
	http.onreadystatechange = function () {
		if (this.readyState === 4 && this.status === 200) {
			...
			document.getElementById("pitch").innerHTML += matchInfo.kickOffTeamStatistics.shots.on + " Shots On Target " + matchInfo.secondTeamStatistics.shots.on + "<br>";
			...
			if (its > (totalIterations / 2) && its < ((totalIterations / 2) + iterations + 1)) {
				switchSides();
	...
	http.onreadystatechange = function () {
		if (this.readyState === 4 && this.status === 200) {
			...
			for (i = 2; i < result.positions.length - 1; i++) {
				...
				ctx.arc(result.positions[i] + 1, result.positions[i + 1] + 1, 8, 0, 2 * Math.PI);
				// ctx.lineTo(result.positions[i] + 1, result.positions[i + 1] + 1);
				if (result.matchjson.half == 1) {
					if (i < 24) {
						ctx.fillStyle = 'red';
...
function movePlayersWatch() {
	...
	http.onreadystatechange = function () {
		if (this.readyState === 4 && this.status === 200) {
			...
			document.getElementById("stats").innerHTML += matchInfo.kickOffTeamStatistics.shots.on + " Shots " + matchInfo.secondTeamStatistics.shots.on + "<br>";
